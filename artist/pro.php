...
include('header.php');
    ...
    $form_msg="";
    $lm=$_SESSION['id'];
    $query1="select * from `artist_reg` where id=".$lm;
	...
	if(isset($_POST['sun']))
	{
		...
		else
		{
			...
			{
				$query3="UPDATE `artist_reg` SET `name` = '$name', `email` = '$email', `password` = '$pwd' WHERE `artist_reg`.`id` = $lm";
				mysqli_query($link,$query3) or die("Error inserting data.".mysqli_error($link));
				$_SESSION['id']=$lm;
					$_SESSION['uname']=$email;
					$_SESSION['name']=$name;
				header("location:artistHome.php?update=true&&id=".$lm);
			}
		}
